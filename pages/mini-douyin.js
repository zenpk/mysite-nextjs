                ...
                <div className="col">
                    ...
                    <div className="content">
                        <ul>
                            ...
                            <li>代码合理分层，有效解耦，可扩展性强。利用较多工具函数减少重复代码</li>
                    ...
                    <div className="content">
                        ...
                        <ul>
                            ...
                            <li>查看用户信息时同时查询是否关注，使关注按钮正常显示</li>
                        </ul>
                    </div>
                    <h3 id="video">视频模块</h3>
                    <div className="content">
                        ...
                        <ul>
                            ...
                            <li>投稿列表获取：根据用户 id 查询用户的投稿列表，先查询 Redis，命中则返回并更新过期时间，未命中再查询 MySQL，并写入 Redis。用户投稿列表在 Redis 中需要单独用一个 Set 存储，以便根据用户 id 查询视频</li>
                        </ul>
                    </div>
                    ...
                    <div className="content">
                        ...
                        <ul>
                            <li>发表评论：将新评论存入 MySQL 同时写入 Redis。评论在 Redis 中的存储相对其他较为复杂，需要同时维护两个数据结构：一个 Set 用于根据视频 id 获取评论 id，一个 Hash 用于根据评论 id 读取评论。为维护数据一致性，对 Redis 中涉及到的视频采用延时双删的方法</li>
                            <li>删除评论：需要额外检验该用户是否有删除评论的权限（即评论对应的用户 id 与当前用户是否相符），删除时对 Redis 中涉及到的评论和视频采用延时双删</li>
                            <li>查看评论列表：根据视频 id 查询所有评论（先查询 Redis，未命中再查询 MySQL 并写入，下同）。查看评论列表的同时需要查询对应的用户信息</li>
                        </ul>
                    </div>
                    ...
                    <div className="content">
                        ...
                        <ul>
                            <li>点赞/取消点赞：操作之前先判断是否合理，即点赞时判断是否已经点过赞，取消点赞时判断是否存在点赞记录。之后再向 MySQL 中添加记录并写入 Redis。一致性维护采用延时双删法</li>
                            <li>查看点赞列表：根据用户 id 查看点赞列表，并返回对应的视频</li>
                        </ul>
                    </div>
                    ...
                    <div className="content">
                        ...
                        <ul>
                            <li>关注/取消关注：操作之前先判断是否合理，即关注时判断是否已经关注过，取消关注时判断是否存在关注记录。之后再向 MySQL 中添加记录并写入 Redis。在 Redis 中为每个用户维护一个 Set 组，key 值包含用户 id，成员为关注用户或粉丝用户 id，由于对 Set 的操作只可能是完整读入或完整删除，因此只要命中 Set 的 key 值，读取到的用户关注粉丝信息即是全部信息</li>
                            <li>查看关注/粉丝列表：先查询 Redis，未命中再查询 MySQL 并写入，需同时查询关注信息</li>
                        </ul>
                    </div>
