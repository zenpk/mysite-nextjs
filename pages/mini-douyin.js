                ...
                <div className="col">
                    ...
                    <div className="content">
                        ...
                        <ul>
                            ...
                            <li>查看评论列表：根据视频 id 查询所有评论（先查询 Redis，未命中再查询 MySQL 并写入，下同）。查看评论列表的同时需要查询对应的用户信息和关注信息</li>
                        </ul>
                    </div>
                    ...
                    <div className="content">
                        ...
                        <ul>
                            <li>点赞/取消点赞：操作之前先判断是否合理，点赞时判断是否已经点过赞，取消点赞时判断是否存在点赞记录。之后再向 MySQL 中添加记录并写入 Redis。一致性维护采用延时双删法</li>
                    ...
                    <div className="content">
                        ...
                        <ul>
                            <li>关注/取消关注：操作之前先判断是否合理，关注时判断是否已经关注过，取消关注时判断是否存在关注记录。之后再向 MySQL 中添加记录并写入 Redis。在 Redis 中为每个用户维护一个 Set 组，key 值包含用户 id，成员为关注用户或粉丝用户 id，由于对 Set 的操作只可能是完整读入或完整删除，因此只要命中 Set 的 key 值，读取到的用户关注粉丝信息即是全部信息</li>
